		 ...
		 <div class="content_wrapper">
		     ...
		     <?php 
		         if(isset($_GET['search'])){
                    ...
                    $pro_query = "SELECT * FROM products WHERE product_keywords LIKE '%$search_query%' ";
				    $pro_result = mysqli_query($connection, $pro_query);
				    if(!$pro_result){
				         	die("Failed " . mysqli_error($connection));
				        }
				    $count = mysqli_num_rows($pro_result);
                    if($count == 0){
         	               echo "<h1 style='color:black; padding:50px;'>NO RESULTS!</h1>";
                         }
                      else { 
